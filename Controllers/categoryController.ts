...
export const createSubCategory = async (req: Request, res: Response) => {
    try {
        ...
        if (theUser) {
            ...
            subCategories.save();
            await CategoryColletction.findOneAndUpdate({ _id: categoryId },
                { $push: { subcategories: subCategories } }, { new: true }
            )
...
export const getAllCategory = async (req: Request, res: Response) => {
    try {
        const theUser = await UserUtils.getUser(req, res);
        if (theUser) {
            const category: any = await CategoryColletction.find().populate('subcategories');
